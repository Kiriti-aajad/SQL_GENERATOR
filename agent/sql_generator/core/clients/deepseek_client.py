"""
DeepSeek Client - FIXED VERSION with Proper Async Timeout Handling
...
class DeepSeekClient:
    """
    FIXED: DeepSeek client with proper async timeout handling
    """
    ...
    def __init__(self, base_url=None, api_key=None, config=None, logger=None):
        ...
        self.api_key = None
        self.model = os.getenv("DEEPSEEK_MODEL_ID", "deepseek-chat")
        ...
        if self.is_offline:
            ...
            self.logger.info(f"DeepSeekClient initialized with model: {self.model}")
    ...
    async def _create_session(self):
        ...
        try:
            if not self.is_offline and (not self.session or self.session.closed):
                ...
                self.logger.info("DeepSeek session created successfully")
        except Exception as e:
            self.logger.error(f"Failed to create DeepSeek session: {e}")
            self.session = None
            self._is_initialized = False
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"DeepSeek health check error: {e}")
            self.health_check_status = False
            return False
            ...
            try:
                ...
                async with self.session.post(  # pyright: ignore[reportOptionalMemberAccess]
                    ...
                    if response.status == 200:
                        ...
                        self.logger.error(f"DeepSeek health check failed: HTTP {response.status}")
                        self.logger.error(f"Response: {response_text}")
                        self.health_check_status = False
                        return False
                        
            except asyncio.TimeoutError:
                self.logger.error("DeepSeek health check timeout")
                self.health_check_status = False
                return False
            except aiohttp.ServerTimeoutError: # pyright: ignore[reportUnusedExcept]
                self.logger.error("DeepSeek health check server timeout")
                self.health_check_status = False
                return False
    ...
    async def generate_sql_async(self, prompt: str, context: Optional[Dict[str, Any]] = None, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                self.logger.error("DeepSeek request timeout (30 seconds)")
                return {
                    "success": False,
                    "generated_sql": "",
                    "sql": "",
                ...
                self.logger.error("DeepSeek server timeout")
                return {
                    "success": False,
                    "generated_sql": "",
                    "sql": "",
            ...
            self.logger.error(f"DeepSeek SQL generation error: {e}")
            return {
                "success": False,
                "generated_sql": "",
                "sql": "",
    ...
    async def close(self):
        ...
        try:
            if self.session and not self.session.closed:
                ...
                self.logger.info("DeepSeek session closed successfully")
        except Exception as e:
            self.logger.error(f"Error closing DeepSeek session: {e}")
        finally:
            self.session = None
            self._is_initialized = False
    ...
    async def validate_configuration(self) -> Dict[str, Any]:
        ...
        return {
            "valid": bool(self.base_url and self.model) and not self.is_offline,
    ...
    def __repr__(self):
        ...
        return f"DeepSeekClient(base_url='{self.base_url or 'not_configured'}', model='{self.model}', status='{status}')"
