"""
Mathstral Client - Schema Searcher Utils Version
- FIXED: All syntax errors and method name formatting
...
- FIXED: Clean and simple code without redundancy
...
class MathstralClient:
    """
    Simple Mathstral client - NO API keys, direct NGROK calls
    """
    ...
    async def async_health_check(self) -> bool:
        """Async health check - NO API keys"""
        try:
            if self.is_offline:
            ...
            # Only Content-Type header needed - NO Authorization
    ...
    async def generate_sql_async(self, prompt: str, **kwargs) -> Dict[str, Any]:
        """Generate SQL - NO API keys, direct NGROK call"""
        try:
            if self.is_offline:
            ...
            async with self.session.post( # pyright: ignore[reportOptionalMemberAccess]
                ...
                if response.status == 200:
                    ...
                    if data and "choices" in data and len(data["choices"]) > 0:
                        first_choice = data["choices"][0]  # FIXED: [0] index
